...
class Py_Button:
    ...
    def render_button(self, window: pygame.Surface):
        render_text = self.text_font.render(self.text, True, 'black')
        pygame.draw.rect(window, self.color, self.Button)
        window.blit(render_text, (self.x_pos + (self.width - render_text.get_width())/2, self.y_pos + (self.height - render_text.get_height())/2))
